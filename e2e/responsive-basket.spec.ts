 ...
 test(`basket remains reachable at ${width}x${height}`, async ({page}) => {
  ...
  const geometry = await page.evaluate(() => {
   ...
   return {overflow:document.documentElement.scrollWidth > innerWidth, left:basket.left, right:basket.right,
    basketBottom:basket.bottom, checkoutHeight:checkout.height, bottom:checkout.bottom, listHeight:lines.height, listBottom:lines.bottom, checkoutTop:checkout.top};
  });
  expect(geometry.overflow).toBe(false);
  expect(Math.abs(geometry.bottom-geometry.basketBottom)).toBeLessThanOrEqual(1);
  expect(geometry.checkoutHeight).toBeLessThan(260);
...
test('empty basket remains open with its checkout controls visible', async ({page}) => {
 await page.setViewportSize({width:1294,height:912});await page.goto('/');
 while(await page.locator('.basket-line').count()) await page.locator('.basket-line').first().getByRole('button',{name:/^حذف .+ من السلة$/}).click();
 await expect(page.locator('.basket')).toBeVisible();
 await expect(page.getByRole('heading',{name:'السلة فارغة',exact:true})).toBeVisible();
 await expect(page.locator('.checkout')).toBeVisible();
 await expect(page.getByRole('button',{name:'الدفع',exact:true})).toBeDisabled();
});
...
test('desktop category rail uses 125px categories and 50px icon action cells',async({page})=>{
 ...
 await expect(cell).toHaveCSS('width','50px');await expect(cell).toHaveCSS('height','50px');await expect(rail).toHaveCSS('width','125px');
 const positions=await page.evaluate(()=>({category:document.querySelector('.category')!.getBoundingClientRect().x,tools:document.querySelector('.rail-tool-cell')!.getBoundingClientRect().x}));expect(positions.tools).toBeLessThan(positions.category);
 ...
 await expect(page.getByRole('button',{name:/سارة حسن/})).toHaveCount(0);await expect(page.locator('.catalog-footer .rail-tool-cell')).toHaveCount(6);await expect(page.getByRole('button',{name:'فئة غير مسماة',exact:true})).toHaveCount(5);
 await page.getByRole('button',{name:'المساعدة',exact:true}).click();await expect(page.getByRole('heading',{name:'المساعدة',exact:true})).toBeVisible();
});
