...
test('store offers require configuration and activation, and production Help excludes demo material',async({page})=>{
  await settings(page);await page.getByRole('tab',{name:'العروض',exact:true}).click();
  await page.getByRole('tab',{name:'المظهر والتجربة',exact:true}).click();await page.getByRole('checkbox',{name:'وضع التجربة',exact:true}).click();await approve(page);await page.getByRole('tab',{name:'العروض',exact:true}).click();
  await field(page,'اسم العرض').fill('عرض المدرسة');
  await approve(page);
  await button(page,'العودة إلى شاشة البيع').click();
  await settings(page);await page.getByRole('tab',{name:'العروض',exact:true}).click();
  await page.getByRole('button',{name:/عرض المدرسة/}).click();await page.getByRole('checkbox',{name:'تفعيل العرض',exact:true}).check();await approve(page);
...
test('printer shortcut opens Hardware in the one Settings workspace and tab drafts survive',async({page})=>{
  ...
  await button(page,'العودة إلى شاشة البيع').click();await settings(page);await expect(field(page,'اسم المتجر')).toHaveValue('متجر موحد');
});
...
test('offer drafts are protected on exit',async({page})=>{
 await settings(page);await page.getByRole('tab',{name:'العروض',exact:true}).click();await field(page,'رمز الكوبون الاختياري').fill('X');
 await button(page,'العودة إلى شاشة البيع').click();await expect(page.getByRole('heading',{name:'حفظ التغييرات؟',exact:true})).toBeVisible();await button(page,'العودة إلى التحرير').click();await expect(field(page,'رمز الكوبون الاختياري')).toHaveValue('X');
 await button(page,'العودة إلى شاشة البيع').click();await button(page,'لا').click();await expect(page.getByRole('dialog')).toHaveCount(0);
});
