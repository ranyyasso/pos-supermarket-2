...
test('thermal settings persist and show a receipt-only preview',async({page})=>{
 ...
 await page.getByRole('button',{name:'إعدادات الطابعة الحرارية',exact:true}).click();
 ...
 await page.getByRole('button',{name:'معاينة وطباعة اختبار',exact:true}).click();
 ...
 await page.getByRole('button',{name:'رجوع من معاينة الطباعة',exact:true}).click();
...
test('thermal printing can be disabled in settings',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'معاينة وطباعة اختبار',exact:true})).toBeDisabled();
 await page.getByRole('button',{name:'حفظ الإعدادات',exact:true}).click();
 await page.reload();
 await page.getByRole('button',{name:'الإعدادات والإدارة',exact:true}).click();
 await page.getByRole('button',{name:'إعدادات الطابعة الحرارية',exact:true}).click();
...
test('unknown barcode can be registered with manager approval and scanned after reload',async({page})=>{
 ...
 await page.getByRole('textbox',{name:'حد المخزون المنخفض',exact:true}).fill('3');
 await page.getByRole('textbox',{name:'المورّد',exact:true}).fill('مورد الاختبار');
 await page.getByRole('button',{name:'حفظ الصنف · موافقة المدير',exact:true}).click();
...
test('manager can edit, disable, enable and delete a registered item',async({page})=>{
 ...
 await page.getByRole('button',{name:'حفظ الصنف · موافقة المدير',exact:true}).click();await page.getByRole('textbox',{name:'رمز المدير',exact:true}).fill('2468');await page.getByRole('button',{name:'اعتماد العملية',exact:true}).click();
 await page.getByRole('button',{name:'الإعدادات والإدارة',exact:true}).click();await page.getByRole('button',{name:'إدارة الأصناف',exact:true}).click();
 let row=page.locator('.managed-product').filter({hasText:'صنف للإدارة'});await row.getByRole('button',{name:'تعديل',exact:true}).click();await page.getByRole('textbox',{name:'اسم الصنف',exact:true}).fill('صنف معدل');await page.getByRole('button',{name:'حفظ التعديلات · موافقة المدير',exact:true}).click();await page.getByRole('textbox',{name:'رمز المدير',exact:true}).fill('2468');await page.getByRole('button',{name:'اعتماد العملية',exact:true}).click();
 await page.getByRole('button',{name:'الإعدادات والإدارة',exact:true}).click();await page.getByRole('button',{name:'إدارة الأصناف',exact:true}).click();row=page.locator('.managed-product').filter({hasText:'صنف معدل'});await row.getByRole('button',{name:'إيقاف',exact:true}).click();await page.getByRole('textbox',{name:'رمز المدير',exact:true}).fill('2468');await page.getByRole('button',{name:'اعتماد العملية',exact:true}).click();await expect(row).toContainText('موقوف');
...
test('inventory receiving and completed sales create stock movements',async({page})=>{
 ...
 await page.getByRole('textbox',{name:'الكمية المستلمة',exact:true}).fill('5');await page.getByRole('textbox',{name:'المورّد / مرجع الاستلام',exact:true}).fill('مورد تجريبي');await page.getByRole('button',{name:'استلام وإضافة للمخزون',exact:true}).click();await page.getByRole('textbox',{name:'رمز المدير',exact:true}).fill('2468');await page.getByRole('button',{name:'اعتماد العملية',exact:true}).click();
 let movements=await page.evaluate(()=>JSON.parse(localStorage.getItem('mizan-inventory-v1')||'{}').movements);expect(movements[0]).toMatchObject({productId:'p1',delta:5,balance:29,type:'receive',supplier:'مورد تجريبي'});
 await page.getByRole('button',{name:'العودة إلى شاشة البيع',exact:true}).click();await page.getByRole('button',{name:/^الدفع/}).click();await page.getByRole('textbox',{name:'المبلغ المستلم نقداً',exact:true}).fill('50000');await page.getByRole('button',{name:'تأكيد الدفع النقدي',exact:true}).click();
...
test('scale barcode applies encoded price and weight, and product labels are printable',async({page})=>{
 await page.getByRole('button',{name:'الإعدادات والإدارة',exact:true}).click();
 await page.getByRole('button',{name:'تسجيل صنف جديد',exact:true}).click();
 await page.getByRole('textbox',{name:'الباركود',exact:true}).fill('777777');await page.getByRole('textbox',{name:'رمز الميزان (٥ أرقام)',exact:true}).fill('12345');await page.getByRole('textbox',{name:'اسم الصنف',exact:true}).fill('تفاح بالوزن');await page.getByRole('combobox',{name:'الوحدة',exact:true}).selectOption('kg');await page.getByRole('textbox',{name:'سعر البيع',exact:true}).fill('10000');await page.getByRole('textbox',{name:'المخزون الابتدائي',exact:true}).fill('10');await page.getByRole('button',{name:'حفظ الصنف · موافقة المدير',exact:true}).click();await page.getByRole('textbox',{name:'رمز المدير',exact:true}).fill('2468');await page.getByRole('button',{name:'اعتماد العملية',exact:true}).click();
 const search=page.getByRole('textbox',{name:'البحث عن منتج أو باركود',exact:true});await search.fill('2012345012509');await search.press('Enter');await expect(page.locator('.basket-line').filter({hasText:'تفاح بالوزن'})).toContainText('وزن 0.125 كغ');await expect(page.locator('.basket-line').filter({hasText:'تفاح بالوزن'})).toContainText('١٬٢٥٠');
...
test('cash tender calculates change and completes a receipt',async({page})=>{
 ...
 await page.getByRole('textbox',{name:'المبلغ المستلم نقداً',exact:true}).fill('50000');
 await page.getByRole('button',{name:'تأكيد الدفع النقدي',exact:true}).click();
 await expect(page.getByRole('dialog')).toContainText('الباقي للعميل');
...
test('partial payments survive reload and finish with card',async({page})=>{
 await page.getByRole('button',{name:'تقسيم الدفع',exact:true}).click();
 await page.getByRole('textbox',{name:'المبلغ المستلم نقداً',exact:true}).fill('20000');
 await page.getByRole('button',{name:'تسجيل الدفعة النقدية',exact:true}).click();
 await page.reload();
 await expect(page.getByRole('dialog')).toContainText('المدفوع');
 await page.getByRole('button',{name:'بطاقة مصرفية',exact:true}).click();
 await page.getByRole('button',{name:'محاكاة موافقة الدفع',exact:true}).click();
 await expect(page.getByRole('heading',{name:'إيصال البيع',exact:true})).toBeVisible();
});
