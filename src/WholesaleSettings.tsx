...
import { saveSalesSettings, type SalesSettings } from './salesSettings';

export function WholesaleSettings({ value, onSave, approve, onDirtyChange }: { onDirtyChange:(dirty:boolean)=>void; value: SalesSettings; onSave: (value: SalesSettings) => void; approve: (title: string, run: () => void) => void }) {
  ...
  function persist(next: SalesSettings) {
    try { saveSalesSettings(next); onSave(next); setMessage('تم حفظ إعدادات الجملة'); }
  ...
  function save() {
    ...
    approve('حفظ سعر الجملة', () => persist({ ...value, wholesale }));
  }
