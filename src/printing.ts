import { money, price, productById, ean13CheckDigit, type Transaction, type Refund } from './model';
...
export const printerDefaults: PrinterSettings = {
  ...
  receiptPrefix: '', logoDataUrl: '', drawerKick: true, footer: 'شكراً لزيارتكم',
};
...
export function salePrintReceipt(tx: Transaction, settings: Pick<PrinterSettings, 'receiptPrefix'> = printerDefaults): PrintReceipt {
  const p = tx.pricing || price(tx.sale);
  const method = {cash: 'نقداً', card: 'بطاقة مصرفية', contactless: 'دفع لاتلامسي'};
  return {
    title: `إيصال #${receiptNumber(tx.sale.id, settings)}`,
    details: [new Date(tx.sale.createdAt).toLocaleString('ar-IQ'), ...(tx.status === 'void' ? ['معاملة ملغاة'] : [])],
    ...
    totals: [
      {label: 'المجموع قبل الخصم', amount: p.subtotal},
      {label: 'الخصومات والعروض', amount: p.itemDiscount + p.promotions + p.basketDiscount + p.reward},
...
export function buildPrintDocument(receipt: PrintReceipt, settings: PrinterSettings, stylesheets: string[] = []): string {
  ...
  <style>
    ...
    *{box-sizing:border-box} .print-controls{padding:16px;background:#eee;color:#111;font-size:14px;line-height:1.8} .print-controls button{min-height:44px;padding:8px 20px;border:1px solid #333;border-radius:4px;background:white;cursor:pointer;font:inherit}
    ...
    @media print{html,body{width:${s.width}mm!important}.print-controls{display:none!important}.thermal-paper{width:${s.width}mm;margin:0;max-width:none;box-shadow:none} }
  </style></head><body>
  <nav class="print-controls" aria-label="أدوات الطباعة"><button id="print-now" type="button">طباعة / حفظ PDF</button>
    <p>اختر ${s.printerName ? `الطابعة «${e(s.printerName)}»` : 'الطابعة الحرارية'} من نافذة الطباعة. اضبط الورق على ${s.width} مم، والمقياس على 100%، والهوامش على «بلا»، وأوقف رؤوس الصفحات وتذييلاتها. عدد النسخ من نافذة الطباعة.</p>
...
export function productLabelDocument(product: {name:string;barcode:string;price:number}, settings: PrinterSettings): string {
  const s=normalizePrinterSettings(settings), e=escapePrintText;
