import { lineName, lineStockQuantity } from './model';
...
export function buildDailyReport(state:State,now=new Date(),from=localDay(now),to=from){
 ...
 const addLines=(tx:typeof completed[number],quantities?:Record<string,number>)=>{
  ...
  for(const line of tx.sale.lines){
   ...
   const direction=quantities?-1:1,ratio=count/line.quantity,quantity=lineStockQuantity(line)*ratio*direction;
   ...
   const key=line.manualName ? `${line.productId}:${line.manualName}` : line.productId;
   const prior=sold.get(key)||{productId:key,name:lineName(line),quantity:0,revenue:0,profit:0,missingCost:false};
   sold.set(key,{...prior,quantity:prior.quantity+quantity,revenue:prior.revenue+revenue,profit:prior.profit+profit,missingCost:prior.missingCost||!known});
  }
 };
 ...
 lowStock:products.filter(p=>!p.untracked&&p.available!==false&&p.stock<=(p.minStock??5)).map(p=>({productId:p.id,name:p.name,stock:p.stock,minimum:p.minStock??5}))};
}
